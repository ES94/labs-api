...
def auth():
    ...
    try:
        ...
        if validJsonData == True:
            ...
            validateArguments = Utils.allPostArgumentsExists(
            ...
            if validateArguments[0] == True:
                ...
                DATABASE = Database.getInstance(
                    __dbHost, __dbUser, __dbPswd, __dbName)
...
@app.route('/getalloptions', methods=['POST'])
            ...
            if token is not None:
                ...
                if 'errno' not in decoded_token:
                    ...
                    if validate_arguments[0]:
                        ...
                        args = (id_client, )
                        opt = db.callProc(
                            Procedure.GET_CLIENT_MENU_OPTIONS,
                            args,
                            True
                        )  # Obtención de opciones
                        ...
                        if opt:
                            response = User.get_ordered_options(opt)
                        else:
                            # Usuario inválido
            ...
            if token is not None:
                ...
                if 'errno' not in decoded_token:
                    ...
                    if validate_arguments[0]:
                        ...
                        id_client = json_data['id_client']
                        args = (id_client, )
                        users_results = db.callProc(
                            Procedure.GET_CLIENT_USERS,
                            args,
                            True
                        )  # Obtención de usuario
                        ...
                        # Revisar que el usuario existe
                        if users_results:
                            response = User.get_users_list(users_results)
                        else:
                            # Usuario inválido
                            error = Error.INVALID_USER
                    else:
            ...
            if token is not None:
                ...
                if 'errno' not in decoded_token:
                    ...
                    if validate_arguments[0]:
                        ...
                        args = (
                            id_client, user_name, email, password,
                            id_hub, options
                        )
